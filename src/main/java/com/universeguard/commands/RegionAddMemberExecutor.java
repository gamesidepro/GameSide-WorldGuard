 ...
 * @author Twelvee
 */
public class RegionAddMemberExecutor implements CommandExecutor {
			...
			if(r != null && r.isOwner(player.getUniqueId())) {
				if(args.hasAny("name")) {
					...
					if(!r.getMembers().contains(p)) {
						r.addMember(p);
						RegionUtils.save(r);
						Utils.sendMessage(player, TextColors.GREEN, "[Игровая Сторона] ", args.<Player>getOne("name").get().getName(), TextColors.WHITE, " добавлен в список жильцов региона!");
					}
					else
						Utils.sendMessage(player, TextColors.RED,"[Игровая Сторона] ", args.<Player>getOne("name").get().getName(), TextColors.WHITE, " уже в списке жильцов региона!");
				}
				else {
