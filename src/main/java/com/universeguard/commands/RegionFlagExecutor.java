			...
			if((r != null && r.isOwner(player.getUniqueId())) || player.hasPermission(Utils.getPermission("bypass"))) {
				if(args.hasAny("flag") && args.hasAny("value")) {
					...
					if(r.setFlag(name, value))
						Utils.sendMessage(player, TextColors.RED, "[Игровая Сторона] ",TextColors.GREEN, "Регион обновлен!");
					else
						Utils.sendMessage(player, TextColors.RED, "[Игровая Сторона] ",TextColors.WHITE, "Флага не существует!");
				}
				else {
					if(!args.hasAny("flag"))
